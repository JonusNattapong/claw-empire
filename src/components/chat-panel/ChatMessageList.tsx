        ...
        <>
          {visibleMessages.map((msg) => {
            ...
            const senderName = isCeo
              ? tr("CEO", "CEO", "CEO", "CEO", "CEO")
              : isSystem
                ? tr("시스템", "System", "システム", "系统", "ระบบ")
                : getAgentName(senderAgent) || senderNameFromPayload || tr("알 수 없음", "Unknown", "不明", "未知", "ไม่ทราบชื่อ");
                    ...
                    {decisionRequest && (
                      <div className="rounded-xl border border-indigo-500/30 bg-indigo-500/10 px-2 py-2">
                        <p className="text-[11px] font-medium text-indigo-200">
                          {tr("의사결정 요청", "Decision request", "意思決定リクエスト", "决策请求", "คำขอตัดสินใจ")}
                        </p>
                        ...
                        >
                          {tr("직접 답변 작성", "Write custom reply", "カスタム返信を作成", "编写自定义回复", "เขียนตอบกลับเอง")}
                        </button>
                      </div>
                    )}
            ...
            if (isCeo) {
              return (
                <div key={msg.id} className="flex flex-col items-end gap-1">
                  <span className="px-1 text-xs text-gray-500">{tr("CEO", "CEO", "CEO", "CEO", "CEO")}</span>
