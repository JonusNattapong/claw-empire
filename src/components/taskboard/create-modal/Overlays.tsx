        ...
        >
          ...
          >
            ...
            <div className="space-y-2 px-4 py-4">
              <p className="text-sm text-slate-200">
                {t({
                  ...
                  zh: "检测到之前输入的数据，是否加载？",
                  th: "มีข้อมูลที่เคยป้อนไว้ ต้องการโหลดหรือไม่?",
                })}
              </p>
              <p className="text-xs text-slate-400">
                {t({
                  ...
                  ja: "最近の下書き（最大3件）",
                  zh: "最近草稿（最多3个）",
                  th: "แบบร่างล่าสุด (สูงสุด 3 รายการ)",
                })}
              </p>
            ...
            <div className="flex justify-end gap-2 border-t border-slate-700 px-4 py-3">
              ...
              >
                {t({ ko: "새로 작성", en: "Start Fresh", ja: "新規作成", zh: "重新填写", th: "เริ่มใหม่" })}
              </button>
              <button
                type="button"
              ...
              >
                {t({ ko: "불러오기", en: "Load", ja: "読み込み", zh: "加载", th: "โหลด" })}
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-2 px-4 py-4">
              <p className="text-sm text-slate-200">
                {t({
                  ...
                  zh: "要在不关联项目的情况下创建任务吗？",
                  th: "สร้างงานนี้โดยไม่เชื่อมโยงโปรเจกต์?",
                })}
              </p>
              <p className="text-xs text-slate-400">
                {t({
                  ...
                  zh: "该任务不会出现在项目历史中。",
                  th: "งานนี้จะไม่ปรากฏในประวัติโปรเจกต์",
                })}
              </p>
            </div>
            <div className="flex justify-end gap-2 border-t border-slate-700 px-4 py-3">
              ...
              <button
                ...
                className="rounded-md bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-blue-500"
              >
                {t({ ko: "계속", en: "Continue", ja: "続行", zh: "继续", th: "ดำเนินการต่อ" })}
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-2 px-4 py-4">
              <p className="text-sm text-slate-200">
                {t({
                  ...
                  zh: "该路径不存在。现在创建吗？",
                  th: "เส้นทางนี้ไม่มีอยู่ ต้องการสร้างตอนนี้หรือไม่?",
                })}
              </p>
              ...
              {missingPathPrompt.nearestExistingParent && (
                <p className="text-xs text-slate-400">
                  {t({
                    ko: `기준 폴: ${missingPathPrompt.nearestExistingParent}`,
                    ...
                    th: `โฟลเดอร์พื้นฐาน: ${missingPathPrompt.nearestExistingParent}`,
                  })}
                </p>
              )}
              {!missingPathPrompt.canCreate && (
                <p className="text-xs text-amber-300">
                  {t({
                    ...
                    th: "ไม่สามารถสร้างเส้นทางนี้ได้ด้วยสิทธิ์ปัจจุบัน เลือกเส้นทางอื่น",
                  })}
                </p>
              )}
            </div>
          ...
          >
            ...
            <div className="space-y-3 px-4 py-4">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {t({ ko: "상위 폴더", en: "Up", ja: "上位フォルダ", zh: "上级目录", th: "ขึ้นไปด้านบน" })}
                </button>
                <button
                  type="button"
        ...
        >
          ...
          >
              ...
              {drafts.length === 0 ? (
                <div className="rounded-lg border border-slate-700 bg-slate-800/70 px-3 py-4 text-center text-sm text-slate-400">
                  {t({
                    ...
                    ja: "保存された一時項目はありません。",
                    zh: "没有已保存的临时草稿。",
                    th: "ไม่มีแบบร่างชั่วคราวที่บันทึกไว้",
                  })}
                </div>
              ) : (
                drafts.map((draft) => (
            ...
            <div className="flex justify-end border-t border-slate-700 px-4 py-3">
              ...
              >
                {t({ ko: "전체 삭제", en: "Delete All", ja: "すべて削除", zh: "全部删除", th: "ลบทั้งหมด" })}
              </button>
            </div>
          </div>
        </div>
