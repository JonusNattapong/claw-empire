    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="space-y-3 px-4 py-4">
          <div className="rounded-lg border border-slate-700 bg-slate-800/70 px-3 py-2">
            <p className="text-[11px] text-slate-400">
              {t({ ko: "현재 위치", en: "Current Location", ja: "現在位置", zh: "当前位置", th: "ตำแหน่งปัจจุบัน" })}
            </p>
          ...
          {manualPathTruncated && (
            <p className="text-[11px] text-slate-400">
              {t({
                ...
                zh: "目录过多，仅显示前300个。",
                th: "แสดงเฉพาะ 300 ไดเรกทอรีแรกเท่านั้น",
              })}
            </p>
          )}
        </div>
        <div className="flex justify-end gap-2 border-t border-slate-700 px-4 py-3">
          ...
          >
            {t({ ko: "현재 폴더 선택", en: "Select Current Folder", ja: "現在フォルダを選択", zh: "选择当前文件夹", th: "เลือกโฟลเดอร์ปัจจุบัน" })}
          </button>
        </div>
      </div>
    </div>
