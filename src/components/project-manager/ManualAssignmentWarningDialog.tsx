      ...
      >
        <div className="border-b border-amber-500/30 px-4 py-3">
          <h3 className="text-sm font-semibold text-amber-200">
            {t({ ko: "수동 배정 확인", en: "Manual Assignment Check", ja: "手動割り当て確認", zh: "手动分配确认", th: "การตรวจสอบการมอบหมายด้วยตนเอง" })}
          </h3>
        </div>
          ...
          <p className="text-sm text-slate-100">
            {warning.reason === "no_agents"
              ? t({
                  ...
                  th: "ไม่ได้เลือกพนักงาน หากบันทึกในสถานะนี้ ตามมาตรการป้องกันโหมดด้วยตนเอง หัวหน้าทีมอาจดำเนินการโดยตรง (คนเดียว) ได้ ต้องการบันทึกต่อหรือไม่?",
                })
              : t({
                  ...
                  th: "เลือกเฉพาะหัวหน้าทีมเท่านั้น ไม่มีพนักงานระดับล่าง ดังนั้นตามมาตรการป้องกันโหมดด้วยตนเอง หัวหน้าทีมอาจดำเนินการโดยตรง (คนเดียว) ได้ ต้องการบันทึกต่อหรือไม่?",
                })}
          </p>
          <div className="rounded-md border border-slate-700 bg-slate-800/60 px-3 py-2 text-[11px] text-slate-300">
            <p>
              {t({ ko: "선택 요약", en: "Selection Summary", ja: "選択サマリー", zh: "选择摘要", th: "สรุปการเลือก" })}: {stats.total}
            </p>
            <p>
              {t({ ko: "팀장", en: "Leaders", ja: "リーダー", zh: "组长", th: "หัวหน้าทีม" })}: {stats.leaders} ·{" "}
              {t({ ko: "하위 직원", en: "Subordinates", ja: "サブ担当", zh: "下属成员", th: "พนักงานระดับล่าง" })}: {stats.subordinates}
            </p>
          </div>
        </div>
        <div className="flex justify-end gap-2 border-t border-slate-700 px-4 py-3">
          ...
          >
            {t({ ko: "계속 저장", en: "Save Anyway", ja: "そのまま保存", zh: "仍然保存", th: "บันทึกต่อ" })}
          </button>
        </div>
      </div>
    </div>
