    ...
    <div className="taskboard-shell flex h-full flex-col gap-4 bg-slate-950 p-3 sm:p-4">
      <div className="flex flex-wrap items-center gap-3">
        <h1 className="text-xl font-bold text-white">
          {t({ ko: "업무 보드", en: "Task Board", ja: "タスクボード", zh: "任务看板", th: "กระดานงาน" })}
        </h1>
        <span className="rounded-full bg-slate-800 px-2.5 py-0.5 text-xs text-slate-400">
          {t({ ko: "총", en: "Total", ja: "合計", zh: "总计", th: "รวม" })} {filteredTasks.length}
          {t({ ko: "개", en: "", ja: "件", zh: "项", th: "รายการ" })}
          {activeFilterCount > 0 &&
        ...
        <div className="ml-auto flex items-center gap-2">
          {activeFilterCount > 0 && (
            ...
            >
              {t({ ko: "필터 초기화", en: "Reset Filters", ja: "フィルターをリセット", zh: "重置筛选", th: "รีเซ็ตตัวกรอง" })}
            </button>
          )}
          <button
          ...
          >
            <span className={showAllTasks ? "text-slate-400" : "text-emerald-200"}>
              {t({ ko: "진행중", en: "Active", ja: "進行中", zh: "进行中", th: "กำลังดำเนินการ" })}
            </span>
          ...
          >
            🗂 {t({ ko: "프로젝트 관리", en: "Project Manager", ja: "プロジェクト管理", zh: "项目管理", th: "จัดการโปรเจคต์" })}
          </button>
          <button
