...
export default function ProjectManagerModal({ agents, departments = [], onClose }: ProjectManagerModalProps) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    const confirmed = window.confirm(
      t({
        ...
        th: `ลบโปรเจคต์ '${selectedProject.name}' หรือไม่?`,
      }),
    );
    if (!confirmed) return;
  ...
  const headerTitle = t({ ko: "프로젝트 관리", en: "Project Management", ja: "プロジェクト管理", zh: "项目管理", th: "จัดการโปรเจคต์" });
  const formTitle = editingProjectId
    ? t({ ko: "프로젝트 수정", en: "Edit Project", ja: "プロジェクト編集", zh: "编辑项目", th: "แก้ไขโปรเจคต์" })
    : isCreating
      ? t({ ko: "신규 프로젝트 등록", en: "Register New Project", ja: "新規プロジェクト登録", zh: "新建项目", th: "สร้างโปรเจคต์ใหม่" })
      : t({ ko: "프로젝트 정보", en: "Project Info", ja: "プロジェクト情報", zh: "项目信息", th: "ข้อมูลโปรเจคต์" });
