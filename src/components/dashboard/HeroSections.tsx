        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs" style={{ color: "var(--th-text-muted)" }}>
            {t({
              ...
              zh: "代理正在实时执行任务",
              th: "เอเจนต์กำลังทำภารกิจแบบเรียลไทม์",
            })}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {reviewQueue > 0 && (
            <span className="flex items-center gap-1.5 rounded-lg border border-orange-400/30 bg-orange-500/15 px-3 py-1.5 text-xs font-bold text-orange-300 animate-neon-pulse-orange">
              🔔 {t({ ko: "대기", en: "Queued", ja: "待機", zh: "待处理", th: "รอดำเนินการ" })}{" "}
              {numberFormatter.format(reviewQueue)}
              {t({ ko: "건", en: "", ja: "件", zh: "项", th: "รายการ" })}
            </span>
          )}
        </div>
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="dashboard-ranking-gradient text-lg font-black uppercase tracking-wider">
              {t({ ko: "랭킹 보드", en: "RANKING BOARD", ja: "ランキングボード", zh: "排行榜", th: "กระดานอันดับ" })}
            </h2>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px]" style={{ color: "var(--th-text-muted)" }}>
                        {agent.department ||
                          t({ ko: "미지정", en: "Unassigned", ja: "未指定", zh: "未指定", th: "ไม่ระบุ" })}
                      </p>
                    </div>
