    ...
    <section className="space-y-4 rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 sm:p-5">
      ...
      {oauthResult && (
        ...
        >
          <span>
            {oauthResult.error
              ? `${t({ ko: "OAuth 연결 실패", en: "OAuth connection failed", ja: "OAuth 接続失敗", zh: "OAuth 连接失败", th: "การเชื่อมต่อ OAuth ล้มเหลว" })}: ${oauthResult.error}`
              : `${OAUTH_INFO[oauthResult.provider || ""]?.label || oauthResult.provider} ${t({ ko: "연결 완료!", en: "connected!", ja: "接続完了!", zh: "连接成功!", th: "เชื่อมต่อสำเร็จ!" })}`}
          </span>
          ...
          <span>
            {oauthStatus.storageReady
              ? t({
                  ...
                  th: "ที่เก็บ OAuth ใช้งานได้ (กำหนดคีย์การเข้ารหัสแล้ว)",
                })
              : t({
                  ...
                  th: "ไม่ได้ตั้งค่าตัวแปรสภาพแวดล้อม OAUTH_ENCRYPTION_SECRET",
                })}
          </span>
        </div>
      )}
